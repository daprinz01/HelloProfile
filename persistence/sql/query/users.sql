...
and language_id == (select d.id from languages d where d.id == $4 or d."name" == $4) returning *;
...
timezone_id = (select b.id from timezones b where b.id == $2 or b."name" == $2) where user_id == (select c.id from users c where c.username == $3 or c.email == $3 limit 1) returning *;
...
identity_provider_id = (select b.id from identity_providers b where b.id == $2 or b."name" == $2) where user_id == (select c.id from users c where c.id == $1 or c."username" == $1 or c.email == $1 limit 1) and identity_provider_id == (select d.id from identity_providers d where d.id == $2 or d."name" == $2)  returning *;
...
and role_id == (select d.id from roles d where d.id == $4 or d."name" == $4) returning *;
