...
func (env Env) ValidateFireBaseToken(c echo.Context) (err error) {
	...
	if request.GoogleJWT == "" {
		errorResponse.Errorcode = util.MODEL_VALIDATION_ERROR_CODE
		errorResponse.ErrorMessage = util.MODEL_VALIDATION_ERROR_MESSAGE
		log.WithFields(fields).WithError(err).WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error("Error occured while trying to marshal request")
		c.JSON(http.StatusBadRequest, errorResponse)
		return fmt.Errorf("auth token was not found in request")
	}
	//verify token
	token, err := firebaseAuth.VerifyIDToken(context.Background(), request.GoogleJWT)
	if err != nil {
		errorResponse.Errorcode = util.UNAUTHORIZED_ERROR_CODE
		errorResponse.ErrorMessage = util.UNAUTHORIZED_ERROR_MESSAGE_WRONG_JWT
		log.WithFields(fields).WithError(err).WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error("The firebase token passed was not valid")
		c.JSON(http.StatusBadRequest, errorResponse)
		return err
	}
	...
	if err != nil {
		errorResponse.Errorcode = util.UNAUTHORIZED_ERROR_CODE
		errorResponse.ErrorMessage = util.UNAUTHORIZED_ERROR_MESSAGE_WRONG_JWT
		log.WithFields(fields).WithError(err).WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error("Firebase user was not found. Authentication failed.")
		c.JSON(http.StatusBadRequest, errorResponse)
		return err
	}
	user, err := env.HelloProfileDb.GetUser(context.Background(), sql.NullString{String: strings.ToLower(firebaseUser.Email), Valid: true})
	if err != nil {
		if err.Error() == "sql: no rows in result set" {
			//Create user
			var firstName, lastName string
			displayName := strings.Split(firebaseUser.DisplayName, " ")
			...
			user, err := env.HelloProfileDb.CreateUser(context.Background(), helloprofiledb.CreateUserParams{
				...
				Username:         sql.NullString{String: firebaseUser.Email, Valid: true},
