...
// 		log.WithField("microservice", "helloprofile.service").Info("Checking application")
...
// 			log.WithField("microservice", "helloprofile.service").WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error(util.APPLICATION_NOT_SPECIFIED_ERROR_MESSAGE)
// 			c.JSON(http.StatusBadRequest, errorResponse)
// 			return nil
// 		}
// 		fields := log.Fields{"microservice": "helloprofile.service", "application": application}
...
func TrackResponseTime(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// Write it to the log
		log.WithField("microservice", "helloprofile.service").Info(fmt.Sprintf("Request executed in %v", responseTime))
		return nil
	}
...
func Authorize(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		log.WithField("microservice", "helloprofile.service").Info("Checking authorization...")
		errorResponse := new(models.Errormessage)
		...
		if len(authArray) != 2 {
			...
			log.WithField("microservice", "helloprofile.service").WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error(util.INVALID_AUTHENTICATION_SCHEME_ERROR_MESSAGE)
		...
		if err != nil || verifiedClaims.Email == "" {
			...
			log.WithField("microservice", "helloprofile.service").WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).WithError(err).Error("Token has expired...")
			c.JSON(http.StatusUnauthorized, errorResponse)
			return nil
		}
...
func AuthorizeAdmin(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		log.WithField("microservice", "helloprofile.service").Info("Checking admin authorization...")
		errorResponse := new(models.Errormessage)
		...
		if err != nil || verifiedClaims.Email == "" {
			errorResponse.Errorcode = util.SESSION_EXPIRED_ERROR_CODE
			errorResponse.ErrorMessage = util.SESSION_EXPIRED_ERROR_MESSAGE
			log.WithField("microservice", "helloprofile.service").WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error("Token has expired...")
			c.JSON(http.StatusUnauthorized, errorResponse)
			return err
		}
		if !(strings.Contains(strings.ToLower(verifiedClaims.Role), "admin") || strings.Contains(strings.ToLower(verifiedClaims.Role), "superadmin")) {
			...
			log.WithField("microservice", "helloprofile.service").WithFields(log.Fields{"responseCode": errorResponse.Errorcode, "responseDescription": errorResponse.ErrorMessage}).Error(fmt.Sprintf("User is not authorised to perform this operation with role(s) %s...", verifiedClaims.Role))
			c.JSON(http.StatusUnauthorized, errorResponse)
			return nil
		}
		// Call the next handler, which can be another middleware in the chain, or the final handler.
...
func Ping(c echo.Context) (err error) {
	fields := log.Fields{"microservice": "helloprofile.service", "application": "backend", "function": "Ping"}
